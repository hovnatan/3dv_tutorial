...
#define Rx(rx)                                                                 \
  (cv::Mat_<double>(3, 3) << 1, 0, 0, 0, cos(rx), -sin(rx), 0, sin(rx), cos(rx))
#define Ry(ry)                                                                 \
  (cv::Mat_<double>(3, 3) << cos(ry), 0, sin(ry), 0, 1, 0, -sin(ry), 0, cos(ry))
#define Rz(rz)                                                                 \
  (cv::Mat_<double>(3, 3) << cos(rz), -sin(rz), 0, sin(rz), cos(rz), 0, 0, 0, 1)

int main() {
  // The given camera configuration: focal length, principal point, image
  // resolution, position, and orientation
  double f = 1000, cx = 320, cy = 240, noise_std = 1;
  cv::Size img_res(640, 480);
  std::vector<cv::Point3d> cam_pos = {
      cv::Point3d(0, 0, 0), cv::Point3d(-2, -2, 0), cv::Point3d(2, 2, 0),
      cv::Point3d(-2, 2, 0), cv::Point3d(2, -2, 0)};
  std::vector<cv::Point3d> cam_ori = {cv::Point3d(0, 0, 0),
                                      cv::Point3d(-CV_PI / 12, CV_PI / 12, 0),
                                      cv::Point3d(CV_PI / 12, -CV_PI / 12, 0),
                                      cv::Point3d(CV_PI / 12, CV_PI / 12, 0),
                                      cv::Point3d(-CV_PI / 12, -CV_PI / 12, 0)};
  ...
  for (size_t i = 0; i < cam_pos.size(); i++) {
    ...
    // Project the points (c.f. OpenCV provides 'cv::projectPoints()' with
