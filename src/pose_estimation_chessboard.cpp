...
int main() {
  ...
  std::vector<cv::Point3d> box_upper = {
      cv::Point3d(4 * board_cellsize, 2 * board_cellsize, -board_cellsize),
  ...
  while (true) {
    ...
    bool success = cv::findChessboardCorners(image, board_pattern, img_points,
    ...
    if (success) {
      ...
      for (int i = 0; i < line_lower.rows; i++)
        cv::line(image, cv::Point(line_lower.row(i)),
                 cv::Point(line_upper.row(i)), cv::Vec3b(0, 255, 0), 2);
